...
bot.on('message', message => {
    ...
    if(command === settings.prefix + settings.rainbowcommand) {
        ...
        if(!message.guild.member(message.author.id).hasPermission("ADMINISTRATOR")) return message.channel.send(settings.messageresponse.memberperm).catch(err=> message.channel.send("no response"))
