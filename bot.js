...
bot.on('message', message => {
	...
	const rolex = message.mentions.roles.first() || message.guild.roles.find(r=> r.name === args [0])
        ...
        if(!color1) return message.channel.send("Please input a first color to color the specified role").catch(err=> message.channel.send("No response"))
    	if(!color2) return message.channel.send("Please input a second color to color the specified role").catch(err=> message.channel.send("No response"))
	if(!color3) return message.channel.send("Please input a thirth color to color the specified role").catch(err=> message.channel.send("No response"))
	if(!color4) return message.channel.send("Please input a fourth color to color the specified role").catch(err=> message.channel.send("No response"))
	if(!color5) return message.channel.send("Please input a fifth color to color the specified role").catch(err=> message.channel.send("No response"))
	let botrole = message.guild.member(bot.user.id).highestRole;
        if(rolex.position > botrole.position){ return message.channel.send("I can't edit that role ! Put my highest role above the role you want me to manage .") }
	if(!delay) return message.channel.send(settings.messageresponse.delaynotfound).catch(err=> message.channel.send("No response"))
        if(!rolex) return message.channel.send(settings.messageresponse.rolenotfound).catch(err=> message.channel.send("No response"))
